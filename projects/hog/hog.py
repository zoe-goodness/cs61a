...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    player_score_oneth = player_score % 10
    tenth_opponent_score = opponent_score // 10 % 10
    return max(1, 3 * abs(player_score_oneth - tenth_opponent_score))
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    max_index = 1
    max_value = make_averaged(roll_dice, times_called)(1, dice)
    for i in range(2, 11):
        value = make_averaged(roll_dice, times_called)(i, dice)
        if value > max_value:
            max_index = i
            max_value = value
    return max_index
